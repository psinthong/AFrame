...
class AsterixConnector(Connector):
    def __init__(self, server_address="localhost:19002", config_file_path="sql_pp.ini"):
        self._db = self.connect(server_address)
        self._host = server_address.split(':')[0]
        self._port = server_address.split(':')[1]
    ...
    def to_collection(self, subquery, namespace, collection, name, query=False):
        new_query = 'CREATE TYPE $namespace._internalType IF NOT EXISTS AS { _uuid: uuid};\n' \
    ...
    def to_view(self, subquery, namespace, collection, name, query=False):
        new_query = 'CREATE OR REPLACE VIEW $namespace.$name AS $subquery;'
        new_query = new_query.replace('$namespace', namespace).replace('$name', name).replace('$subquery', subquery)
        if query:
            return new_query
        self.submit(new_query)
    ...
    def drop_view(self, namespace, collection):
        drop_query = 'DROP VIEW {}.{};'.format(namespace, collection)
        return self.submit(drop_query)
    ...
    def get_collection(self, dataverse, dataset):
        query = 'SELECT VALUE ds FROM Metadata.`Dataset` ds' \
                ' WHERE ds.DatasetName = \"{}\" AND ds.DataverseName = \"{}\";'.format(dataset, dataverse)
    ...
    def get_view(self, dataverse, dataset):
        query = 'SELECT VALUE t FROM Metadata.`Dataset` t  WHERE t.DatasetType = "VIEW" ' \
               'AND t.DataverseName = \"{}\" AND t.DatasetName = \"{}\";'.format(dataverse, dataset)
        result = self.send_request(query)
...
class SQLConnector(Connector):
    ...
    def to_collection(self, subquery, namespace, collection, name, query=False):
        new_query = 'CREATE TABLE $name AS SELECT * FROM ($subquery) t;'
    ...
    def to_view(self, subquery, namespace, collection, name, query=False):
        new_query = 'CREATE OR REPLACE VIEW $name AS $subquery;'
        new_query = new_query.replace('$name', name).replace('$subquery', subquery)
        if query:
            return new_query
        self.submit(new_query)
    ...
    def drop_view(self, namespace, collection):
        drop_query = 'DROP VIEW {};'.format(collection)
        return self.submit(drop_query)

    def drop_transformation(self, namespace, name):
        drop_query = 'DROP FUNCTION {};'.format(name)
        return self.submit(drop_query)

    def get_view(self, dataverse, dataset):
        query= 'SELECT EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = \'{}\');'.format(dataset)
        result = self.send_request(query)
        if result.values[0][0]:
            return 'SELECT * FROM {}'.format(dataset)
        else:
...
class MongoConnector(Connector):
    ...
    def read_json(self, path, name, namespace, orient='records', lines=True, query=False):
        if orient != 'records' and not lines:
            raise ValueError('Only record oriented is supported.')
        if name is None:
            raise ValueError('Require collection name')
        ...
        new_query = self.get_config_queries()['q1']
        return new_query
...
class CypherConnector(Connector):
    ...
    def to_collection(self, subquery, namespace, collection, name, query=False):
        new_query = self.get_config_queries()['to_collection']
        new_query = new_query.replace('$collection', name).replace('$subquery', subquery)
        if query:
            return new_query
        self._db.run(new_query)
        return CypherConnector(uri=self._uri, username=self._username, password=self._password)
    ...
    def drop_collection(self, namespace, collection):
        query = self.get_config_queries()['drop_collection']
    ...
    def read_csv(self, path, sep, name, namespace, header=0, query=False):
        ...
        new_query = new_query.replace('$path', path).replace('$delimiter', sep)
        return new_query
